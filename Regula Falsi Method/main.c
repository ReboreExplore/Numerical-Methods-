/*A C program to implement the Regula Falsi Method or Method of False Position which can be used to find the roots of
...
int main()
{
   ...
   for(j=1;j<4;j++)
   {
       ...
       scanf("%e",&e);
        if(f(a)*f(b)>0)
            {
                printf("\n\nOoops!! No roots lies between the guessed values.");
                continue;
            }
        else{
                break;
